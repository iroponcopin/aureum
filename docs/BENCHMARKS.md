...
- クラス別内訳(histogram、off-1 → on-1)。**この内訳が 2 つの最適化の寄与を分離する**
  — resident の A/B は両方同時に切り替えているので、寄与の分離はここでしかできない:
  ...
  - 内訳合計 **−7.19 MB** に対し、実測の使用ヒープ差は **−6.3 MB**。差の約 0.9 MB は
    ラン間で違う生存データ(このペアではエンティティ 29 対 31 体など)。
    内訳が実測より大きく出ている以上、**−6.3 MB のほうを採用値とする**。
  - 寄与の分離: 検出器側 **−5.20 MB** / 状態キャッシュ側 **−2.00 MB**(0.63+1.04+0.52−0.19)。
    **どちらも単独で正の寄与**であり、片方が全体を担いでいるわけではない。
